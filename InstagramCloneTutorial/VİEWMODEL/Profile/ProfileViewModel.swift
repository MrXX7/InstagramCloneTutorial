...
class ProfileViewModel: ObservableObject {
    ...
    func checkStats() {
        ...
        Firestore.firestore().collection("followers").document(userId).collection("user-followers").getDocuments {
            (snap, err) in
            if let err = err {
                print(err.localizedDescription)
                return
            ...
            Firestore.firestore().collection("following").document(userId).collection("user-following").getDocuments {
                (snap, err) in
                if let err = err {
                    print(err.localizedDescription)
                    return
                ...
                Firestore.firestore().collection("posts").whereField("ownerUid", isEqualTo: userId).getDocuments {
                    (snap, err) in
                    if let err = err {
                        print(err.localizedDescription)
                        return
